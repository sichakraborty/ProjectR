...
strangerplot <- ggplot(df, aes(x = Stranger, fill= I("blue") )) +  
  ...
  scale_y_continuous(name="Percent of Shootings by a Stranger", labels=scales::percent)+
...
#attempt to put all columns on one single graph

  # make a new df of just those columns
#library(dplyr)
#cols<- c("Acquaintance", "Stranger", "Gang", "Mass_Shooting", "Not_Reported")
#subdf<-select(df,cols)
