...
View(df$Participant_Type)

## subtract one since 0::Victim so must coerce as.numeric since it's a character

df$Participant_Type<-as.numeric(df$Participant_Type)
df$Participant_Type<-df$Participant_Type-1
View(df$Participant_Type)

## turn -1 to 0 since 0::Victim str count counts 1

df$Participant_Type[df$Participant_Type== -1]<-0
View(df$Participant_Type)

## Change name of column to Victim_count
library(dplyr)
df <- rename(df, Victim_Count = Participant_Type)
