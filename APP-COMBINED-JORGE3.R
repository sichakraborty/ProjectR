...
strangerplot <- ggplot(df, aes(x = Stranger, fill= I("blue") )) +  
  ...
  scale_y_continuous(name="Percent of Shootings Involving a Stranger", labels=scales::percent)+
...
# The percent of shootings acquaintance related?
aplot <- ggplot(df, aes(x = Acquaintance, fill= I("blue") )) +  
  geom_bar(aes(y = (..count..)/sum(..count..))) + 
  scale_y_continuous(name="Percent of Shootings Involving an Acquaintance", labels=scales::percent)+
  geom_text(aes(y = ((..count..)/sum(..count..)), label = scales::percent((..count..)/sum(..count..))), stat = "count", vjust = -0.25)
aplot <- aplot + theme(axis.text.y = element_blank())
aplot <- aplot + theme(axis.ticks.y = element_blank())
aplot <- aplot + theme(legend.text = element_text(size = 8))
aplot
...
g <- grid.arrange(aplot, strangerplot,gangplot,Not_Reportedplot, ncol=3)

ggsave(filename = "Violence_Relationship.pdf", plot = g, width = 6, height = 9,
       units = "in")
